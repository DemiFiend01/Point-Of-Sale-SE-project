...
from .business.Services.OrderService import OrderService
...
def order_detail(request, order_id):
    ...
    if request.method == 'POST':
        ...
        service.add_item(order_id=order.id, menu_item_id=menu_item_id, quantity=quantity)
        return redirect('order_detail', order_id=order.id)
...
class OrderService:
    ...
    def create(self, waiter: User, is_takeaway: bool, table_no: str | None, notes: str | None) -> Order:
        """
        Creates a new order.
        """
    ...
    def remove_item(self, order_id: int, order_item_id: int) -> bool:
        ...
        """
        try:
            order_item = OrderItem.objects.get(pk=order_item_id, order_id=order_id)
